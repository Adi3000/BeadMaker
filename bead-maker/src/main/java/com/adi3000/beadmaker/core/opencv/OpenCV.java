...
import com.adi3000.beadmaker.core.helper.MathHelper;
import com.adi3000.beadmaker.core.helper.ProcessingHelper;
...
public class OpenCV {
	...
	//added by JS
	private boolean nSidedPolygonFound = false;
	public ArrayList<PVector> identifiedPolygon = new ArrayList<PVector>();
	public ArrayList<Contour> contours = new ArrayList<Contour>();
	public Contour contour;
	public float polygonIsSquareThresholdPercentage = 35f;
   ...
   */
  public OpenCV(String pathToImg){
  	initNative();
  	useColor = false;
  ...
  private void loadFromPImage(PImage img){
...
//  }
   ...
   * 			int
   */
	public OpenCV(int width, int height) {
		initNative();
		init(width, height);
	}
...
//  }
  ...
  private void setupWorkingImages(){
		outputImage = ProcessingHelper.createImage(width,height, PConstants.ARGB);
  }
  ...
  private void initNative(){
  	if(!nativeLoaded){
  		...
  		int bitsJVM = Integer.parseInt(System.getProperty("sun.arch.data.model"));
...
//			} else {
	...
	public PImage getSnapshot(Mat m){
		PImage result = ProcessingHelper.createImage(m.width(), m.height(), PConstants.ARGB);
		toPImage(m, result);
		return result;
	}
	...
	//---------------------------------------------------------------------
	//BELOW THIS LINE IS CUSTOM FUNCTIONS I'VE ADDED TO THE OPENCV CODE
	//---------------------------------------------------------------------
	
	public boolean findNSidedPolygon(int sides, int minSize, int maxSize) {
		//ConsoleHelper.PrintMessage("RUNNING findNSidedPolygon");
		this.nSidedPolygonFound = false;		
		identifiedPolygon = new ArrayList<PVector>();
		
		this.contours = findContours(false, true);
		if (!this.contours.isEmpty()) {
			//ConsoleHelper.PrintMessage("contours are populated");
			this.contour = this.contours.get(0).getPolygonApproximation();
			//ConsoleHelper.PrintMessage("contour area is " + this.contour.area());
			if (this.contour.area() > minSize && this.contour.area() < maxSize) {
				//ConsoleHelper.PrintMessage("contour area is within allowable limits");
				identifiedPolygon = this.contour.getPoints();
				if (identifiedPolygon.size() == sides) {
					//if the distance between the diagonals of the polygon
					//are within a certain percentage of each other, 
					//consider it a square-ish polygon.
					//(this prevents odd shapes from yielding false positives)
					//https://www.youtube.com/watch?v=pXs8hIdnnAs
					if (
						MathHelper.isSimilarValue(
							identifiedPolygon.get(0).dist(identifiedPolygon.get(2)),
							identifiedPolygon.get(1).dist(identifiedPolygon.get(3)),
							polygonIsSquareThresholdPercentage)
					) {
						this.nSidedPolygonFound = true;
					} else {
						//ConsoleHelper.PrintMessage("polygon sides are not similar enough in length");
					}
				} else {
					//ConsoleHelper.PrintMessage("Identified polygon does not have " + sides + " sides.");
				}
			} else {
				if (identifiedPolygon.size() == sides) {
					//ConsoleHelper.PrintMessage("contour area (size) is outside the minimum or maximum limits.");
				}
			}
		} else {
			//ConsoleHelper.PrintMessage("contour(s) are empty.");
		}
		if (this.nSidedPolygonFound) {
			//ConsoleHelper.PrintMessage("successfully found a(n) " + sides + "sided polygon");
		}
		return this.nSidedPolygonFound;
	}
}
